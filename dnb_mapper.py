...
def processFile(inputFileName):
    ...
    if schemaData['fileType'].upper() == 'JSON':
        ...
        if percentHeader >= 90:
            if percentHeader != 100:
                print('warning: first row has %s of %s of expected column headers!' % (headerCount, len(schemaData['columns'])))
            schemaData['columns'] = [x.upper() for x in firstRowValues]
        else:
            print('warning: first does not contain the column header! (%s)' % percentHeader)
    ...
    for row in inputFileReader:
        ...
        #--validate csv
            ...
            print('Column mismatch in row %s: expected %s columns, got %s ... "%s"' % (rowCnt, len(schemaData['columns']), len(row), delimiter.join(row)[0:50]))
        elif schemaData['columns'][0].upper() + '|' + schemaData['columns'][1].upper() == (str(row[0]).upper() if row[0] else '') + '|' + (str(row[1]).upper() if row[1] else ''):
        ...
        if dnbFormat == 'UBO':
            ...
            jsonList1, ubo_company_cache = format_UBO_SUBJECT(rowData, ubo_company_cache)
...
def format_UBO2(rowData, ubo_depth_cache):
    ...
    if rowData['BENF_DUNS']:
        relationshipList.append({
            'REL_ANCHOR_DOMAIN': 'DUNS',
            'REL_ANCHOR_KEY': rowData['BENF_DUNS']
        })
...
def format_UBO_SUBJECT(rowData, ubo_company_cache):
    ...
    if rowData['DOM_ULT_DUNS'] and rowData['DOM_ULT_DUNS'] not in ubo_company_cache['parent']: 
    ...
    if rowData['GLBL_ULT_DUNS'] and rowData['GLBL_ULT_DUNS'] not in ubo_company_cache['parent']: 
...
if __name__ == "__main__":
    ...
    argparser.add_argument('-f', '--dnb_format', default=os.getenv('dnb_format'.upper(), None), type=str.upper, help='choose CMPCVF, GCA, UBO or UBO_ALONE')
